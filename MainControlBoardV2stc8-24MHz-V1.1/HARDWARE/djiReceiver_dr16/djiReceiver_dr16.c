...
uint8_t uSysFlag[2] = {Disable, Disable};
...
eCarMode vGetaction(float fAngle,uint16_t lineLength)
{
	...
	if(SET_McNaimWheel_Mode == uSysFlag[1])
	{
		if(lineLength > config_ACTION_START_MINVAL)				//当设置遥感移动最小的幅度,小于设定值时小车模式处于stop
		{
			...
			else if (fAngle<= 270+ 22.5 && fAngle> 225 + 22.5 )
			{
	...
	}else if(SET_NormalWheel_Mode == uSysFlag[1])
	{
		if(lineLength > config_ACTION_START_MINVAL)				//当设置遥感移动最小的幅度,小于设定值时小车模式处于stop
		{
			if((fAngle<= 45 && fAngle>= 0) || ((fAngle > 360 - 45) && fAngle <= 360 ))				//划分每个角度执行的动作
			{
				eCarModeState = Car_AngleL_360;
			}
			else if (fAngle<= 90 + 45 && fAngle> 45)
			{
				eCarModeState = Car_Angle_90;
			}
			else if (fAngle<= 180 + 45 && fAngle> 90 + 45)
			{
				eCarModeState = Car_AngleR_360;
			}
			else //if (fAngle<= 270 + 45 && fAngle> 270 - 45 )
			{
				eCarModeState = Car_Angle_270;
			}
		}
