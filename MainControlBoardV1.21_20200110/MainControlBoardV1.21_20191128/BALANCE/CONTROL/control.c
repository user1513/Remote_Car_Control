...
extern uint8_t uart_data_array[];
...
{    
	...
	{     
		...
		uart_data_array[6] = (Motor_A/2) >> 8;
		uart_data_array[7] = Motor_A/2 ;
		...
		uart_data_array[14] = (Motor_B/2) >> 8;
		uart_data_array[15] = Motor_B/2 ;
		...
		uart_data_array[22] = (Motor_C/2) >> 8;
		uart_data_array[23] = Motor_C/2 ;
		...
		uart_data_array[30] = (Motor_D/2) >> 8;
		uart_data_array[31] = Motor_D/2 ;
...
{ 	
	 ...
	 pwmAplus = VelocityA_KP*(Bias-Last_bias)+VelocityA_KI*Bias;   //增量式PI控制器
	 Pwm += pwmAplus;
...
{ 	
	 static int Bias,Pwm,Last_bias;
	 Bias=Target-Encoder;                //计算偏差
	 pwmBplus = VelocityB_KP*(Bias-Last_bias)+VelocityB_KI*Bias;   //增量式PI控制器
	 Pwm += pwmBplus;
	 if(Pwm>7200)Pwm=7200;
	 if(Pwm<-7200)Pwm=-7200;
	 Last_bias=Bias;	                   //保存上一次偏差 
...
{ 	
	 static int Bias,Pwm,Last_bias;
	 Bias=Target-Encoder;                                  //计算偏差
	 pwmCplus = VelocityC_KP*(Bias-Last_bias)+VelocityC_KI*Bias;   //增量式PI控制器
	 Pwm += pwmCplus;
	 if(Pwm>7200)Pwm=7200;
	 if(Pwm<-7200)Pwm=-7200;
	 Last_bias=Bias;	                   //保存上一次偏差 
...
{ 	
	 static int Bias,Pwm,Last_bias;
	 Bias=Target - Encoder;                                  //计算偏差
	 pwmDplus = VelocityD_KP*(Bias-Last_bias)+VelocityD_KI*Bias;   //增量式PI控制器
	 Pwm += pwmDplus;
	 if(Pwm>7200)Pwm=7200;
	 if(Pwm<-7200)Pwm=-7200;
	 Last_bias=Bias;	                   //保存上一次偏差 
